...
def runGame():
    ...
    while True: # main game loop
        ...
        if wormCoords[HEAD]['x'] == candy['x'] and wormCoords[HEAD]['y'] == candy['y']:
            candy = getRandomLocation()
            FPS += 2
        if wormCoords[HEAD]['x'] > bird_x + 80 - CELLSIZE*3 and wormCoords[HEAD]['x'] < bird_x + 80 + CELLSIZE*3  and wormCoords[HEAD]['y'] < bird_y + 80 + CELLSIZE*3 and wormCoords[HEAD]['y'] > bird_y + 80 - CELLSIZE*3 :
           showYouDiedScreen() 
        ...
        drawApple(apple)
        drawCandy(candy)
        drawBird(birdCoords[0]['x'], birdCoords[0]['y'])
        # if collision():
        #     showGameOverScreen()
...
def drawBird(x,y):
    ...
    x1 = x
    y2 = y
    birdRect = pygame.Rect(x1, y2, CELLSIZE, CELLSIZE)
    pygame.draw.rect(DISPLAYSURF, DARKGRAY, birdRect)
