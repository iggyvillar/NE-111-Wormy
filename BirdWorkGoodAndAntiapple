...
def runGame():
    ...
    while True: # main game loop
        ...
        if wormCoords[HEAD]['x'] == bird['x'] and wormCoords[HEAD]['y'] == bird['y']:
            return
        for wormBody in wormCoords[1:]:
            if wormBody['x'] == bird['x'] and wormBody['y'] == bird['y']:
                FPS = 15
                return
        ...
        if FPS > 14:            
            ...
            newHead1 = {'x': bird['x'] + 1 , 'y': bird['y']}
            bird = newHead1
                
       # if bird['x'] < CELLWIDTH or bird['y'] > CELLHEIGHT or bird['x'] == -1 or bird['y'] == -1:
           # getRandomLocationY()
        if bird['x'] == CELLWIDTH  or bird['y'] == CELLHEIGHT:
                bird = getRandomLocationY()
...
def drawDoublePressKeyMsg():
    doublepressKeySurf = BASICFONT.render('Double Press a key to play.', True, DARKGRAY)
    doublepressKeyRect = doublepressKeySurf.get_rect()
    doublepressKeyRect.topleft = (WINDOWWIDTH - 267, WINDOWHEIGHT - 30)
    DISPLAYSURF.blit(doublepressKeySurf, doublepressKeyRect)
...
def getRandomLocationX():
    return {'x': random.randint(0, CELLWIDTH - 1), 'y': CELLHEIGHT - 35}

def getRandomLocationY():
    return {'x': (CELLWIDTH - 75), 'y': random.randint(0, CELLHEIGHT - 1)}
...
def showGameOverScreen():
    ...
    gameRect.midtop = (WINDOWWIDTH / 2, 250)
    overRect.midtop = (WINDOWWIDTH / 2, gameRect.height + 250 + 10 + 25)
